...
function composePost(item, target, fallback) {
  ...
  return {
    html: `🔔 ${at} — <b>${escHtml(client)}</b>${file}: ${ask}${from}${due}${note}`,
    mentions,
  };
}

// -- orchestration: post new items + escalate open ones ------------------------
async function routeAndPost(token, cfg, state) {
  if (cfg.dryRun || !cfg.teamsChatId) return;
  const henry = cfg.mailbox ? await graph.resolveUserId(token, cfg.mailbox) : null;
  const nowISO = new Date().toISOString();
  let newPosts = 0;

  // Latest inbound reply time per conversation — to detect "answered / no reply yet".
  const latestByConv = {};
  for (const id of Object.keys(agentState.items)) {
    const it = agentState.items[id];
    const t = new Date(it.receivedDate || 0).getTime();
    if (it.conversationId) latestByConv[it.conversationId] = Math.max(latestByConv[it.conversationId] || 0, t);
  }
...
async function syncCmMonitor() {
  ...
  try {
    ...
    stateLib.save(state);
    await routeAndPost(token, cfg, state);
    ...
    console.log(
      ...
      `actionable=${actionable} claudeCalls=${agentState.counters.claudeCallsMade} posts=${agentState.counters.postsSent}`
    );
  } catch (e) {
...
async function debugRoute(limit = 25, classify = false) {
  ...
  for (const m of inbox.slice(0, limit)) {
    ...
    out.push({
      ...
      note: senderIsPoc ? 'suppressed: sender is the POC' : null,
      // Would this post in production? Every non-suppressed action_required posts —
      // to the intake when routed, otherwise to Henry as a review fallback.
      wouldRoute: (classification === 'action_required') && !senderIsPoc,
      dest: (classification === 'action_required') && !senderIsPoc
