...
// "Jack Kartashyan (Intake Specialist)" for the classifier's From/To/Cc lines. The inbox
// owner isn't in the registry and is who the prompt means by "the recipient", so he is
// labelled explicitly; anyone else off-registry is "(staff)" or "(external)" by domain.
function describeAddress(r, cfg) {
  const addr = String(r?.emailAddress?.address || '').toLowerCase();
  const name = r?.emailAddress?.name || addr || 'unknown';
  if (cfg.mailbox && addr === cfg.mailbox.toLowerCase()) return `${name} (Operations Manager (this inbox))`;
  const hit = loadRegistry(cfg).find((s) => String(s.email || '').toLowerCase() === addr);
  if (hit) return `${name} (${hit.role})`;
  return `${name} (${/@melawyers\.com$/.test(addr) ? 'staff' : 'external'})`;
}
...
// Is any Intake Specialist on the email (From/To/Cc)? Routing pings the intake who
// signed the case up, so an ask on a thread with no intake on it (e.g. a disbursement
// question To Henry + accounting) is Henry's own mail, not theirs (#5760).
function intakeOnThread(msg, cfg) {
  const intakeEmails = new Set(loadRegistry(cfg)
    .filter((s) => /intake/i.test(String(s.role || '')))
    .map((s) => String(s.email || '').toLowerCase()));
  return [msg.from, ...(msg.toRecipients || []), ...(msg.ccRecipients || [])]
    .some((r) => intakeEmails.has(String(r?.emailAddress?.address || '').toLowerCase()));
}
...
const CLASSIFY_SYSTEM =
  ...
  'WHO is being asked matters as much as whether there is an ask. The From, To and Cc lines carry ' +
  'each person\'s role; "Operations Manager (this inbox)" is the recipient whose mail you are triaging. ' +
  'An ask counts ONLY when it is aimed at the intake team (an Intake Specialist) or at the Operations ' +
  'Manager. An ask aimed at another department — case managers or CM assistants, property damage, ' +
  'pre-lit, negotiations, litigation, accounting, attorneys — is irrelevant, even when intake is copied. ' +
  'Being on Cc is NOT being asked: when the intake team is only copied and the To line is another ' +
  'department, a question or request put to "everyone" or to the group belongs to the To line — irrelevant. ' +
  'Only the sender\'s new text is shown; quoted history below a reply is never the ask. ' +
  'action = a short imperative summary of the ask, else null.';

// Bump whenever the prompt or its input changes: cached verdicts from an older version
// re-classify on the next sync, so a still-open misfire stops nagging on its own.
// Already-posted items never re-post (their routed record is kept).
...
function isSkippable(msg, cfg) {
  ...
  // RFD emails are generated by chase-ui and sent CM -> Cintia, Cc'ing Henry so he can watch the
  // automation. They land in this inbox looking exactly like a routable ask: the subject opens
  // with a file number, which the parser reads, and "Requesting review for demands on X" is a
  // direct request, which the classifier would call action_required. It is pre-lit workflow and
  // belongs to Cintia — never to the intake who signed the case up. Skipped on the subject
  // because that is the cheapest gate, before the classifier spends a Claude call.
  // Emitted by chase-ui's RFD automation; see brain/29-rfd-automation.md.
  if (subj.includes('review for demands')) return true;
  // Mail FROM the referrals/sub-outs team is sub-out coordination, never an intake action item
  // (Henry, 2026-09-25). The prompt already calls sub-out coordination irrelevant, but their
  // replies are addressed To the intake by name, so the classifier kept reading them as asks
  // (#6719, #6765). Matched on the sender's registry role, before the classifier is paid.
  const sender = String(msg.from?.emailAddress?.address || '').toLowerCase();
  const hit = loadRegistry(cfg).find((s) => String(s.email || '').toLowerCase() === sender);
  if (hit && /referals?|referrals?|subouts?/i.test(String(hit.role || ''))) return true;
  return false;
}
...
async function classifyMessage(msg, cfg, state) {
  ...
  if (cached && cached.v === CLASSIFIER_VERSION) return { type: cached.type, action: null, dueDate: cached.dueDate };
  if (cfg.dryRun || !cfg.baaSigned) return null; // gate: bodies (PHI) → Claude only when live

  // uniqueBody = the sender's new text without the quoted thread; falls back to the
  // full body when Graph returns none.
  const bodyText = stripHtml(msg.uniqueBody?.content || msg.body?.content || msg.bodyPreview || '').slice(0, 6000);
  const list = (rs) => (rs || []).map((r) => describeAddress(r, cfg)).join('; ') || '(none)';
  const header = `From: ${describeAddress(msg.from, cfg)}\nTo: ${list(msg.toRecipients)}\nCc: ${list(msg.ccRecipients)}\n`;
  const resp = await client().messages.create({
    model: MODEL, max_tokens: 300, // no temperature — deprecated for claude-sonnet-5
    system: CLASSIFY_SYSTEM,
    messages: [{ role: 'user', content: `${header}Subject: ${msg.subject || '(no subject)'}\n\n${bodyText}` }],
  });
  ...
  state.classifiedIds[msg.id] = { type: parsed.type, dueDate: parsed.dueDate || null, v: CLASSIFIER_VERSION };
  return parsed;
}
...
async function buildItems(inbox, cfg, state) {
  ...
  for (const m of inbox) {
    ...
    // No intake on the email → the ask was put to Henry (or another department), and
    // Henry already has it in his inbox. Dropping it from items also stops the
    // reminder/escalation ladder for anything posted before this rule existed.
    if (!intakeOnThread(m, cfg)) {
      console.log(`[cm-monitor] skip no-intake-on-thread msg=${m.id}`);
      continue;
    }
...
async function debugRoute(limit = 25, classify = false) {
  ...
  for (const m of inbox.slice(0, limit)) {
    ...
    const noIntake = !intakeOnThread(m, cfg);
    out.push({
      ...
      note: senderIsPoc ? 'suppressed: sender is the POC'
        : noIntake && classification === 'action_required' ? 'skipped: no intake on the email'
      ...
      wouldRoute: (classification === 'action_required') && !senderIsPoc && !noIntake
        && (!!intakeHit || cfg.reviewFallback),
      dest: (classification === 'action_required') && !senderIsPoc && !noIntake && (!!intakeHit || cfg.reviewFallback)
