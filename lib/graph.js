...
const MSG_SELECT = [
  ...
  'receivedDateTime', 'sentDateTime', 'bodyPreview', 'body', 'uniqueBody', 'webLink',
].join(',');
...
// The seed is base64 of a .graph_token written by an Admin sign-in script.
function decodeDelegatedSeed() {
  if (!process.env.MS_DELEGATED_TOKEN_SEED) return null;
  try {
    return JSON.parse(
      Buffer.from(process.env.MS_DELEGATED_TOKEN_SEED, 'base64').toString('utf8').replace(/^﻿/, ''),
    );
  } catch (_) {
    return null;
  }
...
// One refresh attempt. Returns the token response, or the error code only —
// never the body, which carries the token itself.
async function redeemRefreshToken(refreshToken) {
  const body = new URLSearchParams({
...
async function getDelegatedToken() {
  ...
  let attempt = await redeemRefreshToken(saved.refresh_token);

  // Recovery path. Azure rotates the refresh token on every redemption and the
  // chain dies for good once it lapses — which is what happened on 2026-09-18,
  // taking the CM action router and every queued Teams alert down with it until
  // 2026-09-22. Re-signing in produces a fresh seed, but the block above only
  // writes the seed when NO token file exists, and on Railway the file is on a
  // persistent volume — so a new seed was inert and recovery meant deleting a
  // file inside a running container. Instead: when the stored chain is rejected
  // and the seed holds a DIFFERENT one, take the seed. Pasting the new value
  // into Railway is then the whole repair.
  if (!attempt.ok && attempt.code === 'invalid_grant') {
    const seed = decodeDelegatedSeed();
    if (seed && seed.refresh_token && seed.refresh_token !== saved.refresh_token) {
      console.log('[graph] stored refresh token was rejected — retrying with MS_DELEGATED_TOKEN_SEED');
      attempt = await redeemRefreshToken(seed.refresh_token);
      if (attempt.ok) console.log('[graph] delegated token recovered from the seed');
    }
  }
  if (!attempt.ok) {
    throw new Error(`delegated token refresh failed: ${attempt.status} ${attempt.code} — if invalid_grant, re-auth via an Admin script (authorize-demand-teams.ps1) and paste the new MS_DELEGATED_TOKEN_SEED`);
  }
